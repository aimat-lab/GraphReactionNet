...
def get_bonds(coords, elements, force_bonds=False, forced_bonds=[]):
    ...
    rcov = {
        "H": 0.34, "He": 0.46, "Li": 1.2, "Be": 0.94, "B": 0.77, "C": 0.75, "N": 0.71, "O": 0.63, "F": 0.64, "Ne": 0.67,
        "Na": 1.4, "Mg": 1.25, "Al": 1.13, "Si": 1.04, "P": 1.1, "S": 1.02, "Cl": 0.99, "Ar": 0.96, "K": 1.76,
        "Ca": 1.54, "Sc": 1.33, "Ti": 1.22, "V": 1.21, "Cr": 1.1, "Mn": 1.07, "Fe": 1.04, "Co": 1.0, "Ni": 0.99,
        "Cu": 1.01, "Zn": 1.09, "Ga": 1.12, "Ge": 1.09, "As": 1.15, "Se": 1.1, "Br": 1.14, "Kr": 1.17, "Rb": 1.89,
        "Sr": 1.67, "Y": 1.47, "Zr": 1.39, "Nb": 1.32, "Mo": 1.24, "Tc": 1.15, "Ru": 1.13, "Rh": 1.13, "Pd": 1.19,
        "Ag": 1.15, "Cd": 1.23, "In": 1.28, "Sn": 1.26, "Sb": 1.26, "Te": 1.23, "I": 1.32, "Xe": 1.31, "Cs": 2.09,
        "Ba": 1.76, "La": 1.62, "Ce": 1.47, "Pr": 1.58, "Nd": 1.57, "Pm": 1.56, "Sm": 1.55, "Eu": 1.51, "Gd": 1.52,
        "Tb": 1.51, "Dy": 1.5, "Ho": 1.49, "Er": 1.49, "Tm": 1.48, "Yb": 1.53, "Lu": 1.46, "Hf": 1.37, "Ta": 1.31,
        "W": 1.23, "Re": 1.18, "Os": 1.16, "Ir": 1.11, "Pt": 1.12, "Au": 1.13, "Hg": 1.32, "Tl": 1.3, "Pb": 1.3,
        "Bi": 1.36, "Po": 1.31, "At": 1.38, "Rn": 1.42, "Fr": 2.01, "Ra": 1.81, "Ac": 1.67, "Th": 1.58, "Pa": 1.52,
        ...
        for i in range(len(coords)):
            if i not in indeces_used:
                ...
                for next_atom_idx in next_atom_indeces:
                    ...
                    if elements_bond not in [["Cl", "H"], ["H", "Cl"], ["Cl", "F"], ["F", "Cl"], ["F", "H"], ["H", "F"],
                                             ["Pd", "F"], ["F", "Pd"], ["H", "H"], ["F", "F"], ["Cl", "Cl"]]:
...
def run_crest(coords, elements, moldir, filename, settings, smiles):
    ...
    if crest_done:
        ...
        for conf_idx in range(len(coords_all)):
            ...
            print("   ---   Run xtb calculation of molecule %s, conformer %i out of %i" % (
            filename, conf_idx + 1, len(coords_all)))
            if settings["add_Pd_Cl2_PH3"] or settings["add_Pd_Cl2"] or settings["add_Ni_CO_3"]:
                P_index = settings["P_index"]
                mask, done = get_ligand_indeces(np.array(coords_all[conf_idx]), elements_all[conf_idx], P_index, smiles,
                                                settings["metal_char"])
                if not done:
                    exit()
                if settings["add_Ni_CO_3"] and len(mask) != len(coords_all[conf_idx]) - 7:
                    print("WARNING: expected a mask of length 7 but got %i. Skip this conformer." % (
                                len(coords_all[conf_idx]) - len(mask)))
    ...
    os.chdir(startdir)
    return (crest_done, xtb_done, coords_all_used, elements_all_used, boltzmann_data_used, conf_indeces_used,
            electronic_properties_conformers, [time_crest, time_xtb_sterimol])
    ...
    if os.path.exists("lmocent.coord"):
        for line in open("lmocent.coord", "r"):
            if len(line.split()) == 4 and "He" in line:
                dummy_positions.append([float(line.split()[0]) / AToBohr, float(line.split()[1]) / AToBohr,
                                        float(line.split()[2]) / AToBohr])
...
def read_xtb_log():
    ...
    for line in open("xtb_esp.dat", "r"):
        if len(line.split()) == 4:
            esp_points.append(
                [float(line.split()[0]), float(line.split()[1]), float(line.split()[2]), float(line.split()[3])])

    nucleophilicity = -IP_delta_SCC

    return (
    muls, alphas, wils, dip, alpha, fukui, HOMO_LUMO_gap, IP_delta_SCC, EA_delta_SCC, global_electrophilicity_index,
    esp_profile, esp_points, occ_energies, virt_energies, nucleophilicity)
...
def read_xtb_log1():
    ...
    for line in open("xtb.log", "r"):
        ...
        if read_orbital_energies and "-----" not in line and "..." not in line and len(
                line.split()) != 0 and "Occupation" not in line:
            # print(line)
...
def get_results_conformer():
    ...
    return (True, muls, alphas, wils, dip, alpha, fukui, HOMO_LUMO_gap, IP_delta_SCC, EA_delta_SCC,
            global_electrophilicity_index, esp_profile, esp_points, occ_energies, virt_energies, nucleophilicity)
...
def call_crest(filename, settings):
    ...
    if settings["reduce_output"]:
        for x in os.listdir("."):
            if x.startswith("M") or x.startswith(
                    "N") or x == "wbo" or x == "coord" or "_rotamers_" in x or ".tmp" in x or x.startswith(
                    ".") or x == "coord.original":
                if os.path.isdir(x):
                    try:
                        shutil.rmtree(x)
...
def run_sterimol(coords, elements, dummy_positions, moldir, settings, smiles):
    if settings["add_Pd_Cl2_PH3"] or settings["add_Pd_Cl2"] or settings["add_Ni_CO_3"]:
        ...
        neighbor_dummy_distances = scsp.distance.cdist(np.array(dummy_positions)[nearest_dummy_indeces],
                                                       np.array(coords)[neighbor_indeces])
...
def run_morfeus(coords, elements, dummy_positions, moldir, settings, smiles):
    ...
    if settings["add_Pd_Cl2_PH3"] or settings["add_Pd_Cl2"] or settings["add_Ni_CO_3"]:
        ...
        if len(neighbor_indeces) != 3:
            print("WARNING: found %i instead of 3 neighbor indeces for ligand with %s (%s %s)" % (
            len(neighbor_indeces), metal_char, os.getcwd(), moldir))
            do_pyramid = False
    ...
    if do_pyramid:
        try:
            pyr = Pyramidalization(elements=elements_extended, coordinates=coords_extended, atom_index=p_idx + 1,
                                   excluded_atoms=[dummy_idx + 1])  # remove Pd
            pyr_val = float(pyr.P)
    ...
    try:
        ...
        for i in neighbor_indeces:
            bv = BuriedVolume(elements_extended, coords_extended, dummy_idx + 1, excluded_atoms=[dummy_idx + 1],
                              z_axis_atoms=[p_idx + 1], xz_plane_atoms=[i + 1], density=0.01)  # dummy_idx+1 = 2
            bv.octant_analysis()
...
def reduce_data(data_here):
    ...
    for key in data_here["conf_0"]["sterimol_parameters"].keys():
        if key == "elements_extended":
            ...
            # data=[]
            # for confname in confnames:
            #    data.append(data_here[confname]["sterimol_parameters"][key])
            # data=np.array(data)
            # data_averaged=np.average(data, weights=weights, axis=0)
            # data_min=np.min(data, axis=0)
            # data_max=np.max(data, axis=0)
    ...
    for confname in confnames:
        ...
        # del data_here[confname]["sterimol_parameters"]
...
def combine_csvs(molname, resultsdir, data_here, data_here_confs):
    datagroups = ["lval", "B1", "B5", "sasa", "sasa_P", "sasa_volume", "cone_angle",
                  ...
                  "HOMO_LUMO_gap", "IP_delta_SCC", "nucleophilicity", "cone_angle", "p_int", "p_int_atom", "p_int_area",
                  "pyr_val", "pyr_alpha", "qvbur_min", "qvbur_max", "qvtot_min", "qvtot_max", "max_delta_qvbur",
                  "max_delta_qvtot", "ovbur_min", "ovbur_max", "ovtot_min", "ovtot_max", "vbur", "vtot", "near_vbur",
                  "far_vbur", "near_vtot", "far_vtot"]
    ...
    conformers_to_use = np.array(
        [0] + sorted(np.random.choice(list(range(1, n_conformers)), size=N - 1, replace=False).tolist()))
    ...
    electronic_properties = ['EA_delta_SCC', 'HOMO_LUMO_gap', 'IP_delta_SCC', 'alpha', 'alphas',
                             'global_electrophilicity_index', 'muls', 'nucleophilicity', 'wils']
    sterimol_parameters = ['B1', 'B5', 'lval', 'sasa', 'sasa_P', 'sasa_volume', "cone_angle", "p_int", "p_int_atom",
                           "p_int_area", "pyr_val", "pyr_alpha", "qvbur_min", "qvbur_max", "qvtot_min", "qvtot_max",
                           "max_delta_qvbur", "max_delta_qvtot", "ovbur_min", "ovbur_max", "ovtot_min", "ovtot_max",
                           "vbur", "vtot", "near_vbur", "far_vbur", "near_vtot", "far_vtot"]
    ...
    for p in sterimol_parameters:
        ...
        if feature_ref is not None:
            ...
            mask_here = []
            for c_idx in range(0, n_conformers):
                x = data_here_confs["conf_%i" % (c_idx)]["sterimol_parameters"][p]
                if x is None:
                    # print("WARNING: found None in %s of conformer %i"%(p, c_idx))
                    # x = 0.0
                    data_here.append(x)
            ...
            if which("molconvert") != None:
                ...
                if coords is None or elements is None:
                    ...
                    if abs(np.max(coords.T[2]) - np.min(coords.T[2])) > 0.01:
                        ...
                        return (coords, elements)
                    elif elements in [['Cl', 'H'], ['Br', 'H'], ['I', 'H']]:
                        return (coords, elements)
                    else:
...
def get_coords_from_smiles_rdkit(smiles, suffix):
    ...
    try:
        ...
        if np.max(distances) < 0.1:
            ...
            print(f"ERROR: something is wrong with rdkit molecule {smiles}. Exit!")
            print("%i\n"%(len(coords)))
            for atomidx, atom in enumerate(coords):
