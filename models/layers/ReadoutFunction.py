...
class ReadoutFunction(nn.Module):
    ...
    def __set_readout(self, readout_def, args):
        ...
        self.r_function = {
                    ...
                    'mpnn':     self.r_mpnn,
                    'mpnn_global_readout': self.r_mpnn_global,
                    'mpnn_concat' : self.r_mpnn_concat
        ...
        init_parameters = {
            ...
            'mpnn':     self.init_mpnn,
            'mpnn_global_readout': self.init_mpnn_global,
            'mpnn_concat' : self.init_mpnn_concat
    ...
    def r_mpnn(self, h):
        ...
        # For each graph
        for i in range(h[0].size(0)):
            # TODO add global node readout function 
            nn_res = nn.Sigmoid()(self.learn_modules[0](torch.cat([h[0][i,:,:], h[-1][i,:,:]], 1)))*self.learn_modules[1](h[-1][i,:,:])
    ...
    def r_mpnn_global(self, h):
	
        aux = Variable( torch.Tensor(h[0].size(0), self.args['out']).type_as(h[0].data).zero_() )
        # For each graph
        for i in range(h[0].size(0)):
            # TODO add global node readout function 
            nn_res = nn.Sigmoid()(self.learn_modules[0](torch.cat([h[0][i,-1,:][None, ...], h[-1][i,-1,:][None, ...]], 1)))*self.learn_modules[1](h[-1][i,:,:])
            # nn_res = nn.Sigmoid()(self.learn_modules[0](torch.cat([h[0][i,-1,:][None, ...], h[-1][i,-1,:][None, ...]], 1)))*self.learn_modules[1](h[-1][i,:,:])
    ...
    def r_mpnn_concat(self, h):
	
        aux = Variable( torch.Tensor(h[0].size(0), self.args['out']).type_as(h[0].data).zero_() )
        # For each graph
        for i in range(h[0].size(0)):
            # TODO add global node readout function 
            nn_res = nn.Sigmoid()(self.learn_modules[0](torch.cat([h[0][i,:,:], h[1][i,:,:], h[2][i,:,:], h[3][i,:,:]], 1)))*self.learn_modules[1](h[-1][i,:,:])
    ...
    def embedding(self, h):
        ...
        for i in range(h[0].size(0)):
            ...
            # TODO embedde where
            # aux[i, :] = torch.sum(nn_res, 0)
        print(f"nn_res shape: {nn_res.size()}")
        return nn_res
    ...
    def init_mpnn_global(self, params):
        learn_args = []
        learn_modules = []
        args = {}
    ...
    def init_mpnn_concat(self, params):
        learn_args = []
        learn_modules = []
        args = {}

        # i
        learn_modules.append(NNet(n_in=4*params['in'], n_out=params['target']))
