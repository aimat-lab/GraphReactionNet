...
class MessageFunction(nn.Module):
        ...
        for w in range(h_w.size(1)):
            if torch.nonzero(e_vw[:, w, :].data).size():
                for i, el in enumerate(self.args['e_label']):
                    ...
                    m_w = torch.transpose(torch.bmm(torch.transpose(parameter_mat, 1, 2), torch.transpose(torch.unsqueeze(h_w[:, w, :], 1), 1, 2)), 1, 2)
    ...
    def m_mpnn(self, h_v, h_w, e_vw, opt={}):
        ...
        # TODO debug and see this part how it implemented message part
